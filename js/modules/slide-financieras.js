var SlideFinancieras = (function () {
  ...
  events.crearSlide = function (slide_main_wrapper) {
    ...
    var slide_wrapper = $(slide_main_wrapper).find(
      ".SlideFinancieras-slideWrapper"
    );
    var slide = $(slide_main_wrapper).find(".SlideFinancieras-list");
    var buttons_wrapper = $(slide_main_wrapper).find(
      ".SlideFinancieras-listButtons"
    );
    ...
    slide.slick({
      ...
      prevArrow:
        '<div class="slideButton slideButton--prev"><i class="icon"></i></div>',
      nextArrow:
        '<div class="slideButton slideButton--next"><i class="icon"></i></div>',
      speed: 250,
      cssEase: "linear",
      focusOnSelect: false,
      reponsive: false,
    ...
    $(".slideButton").on("click", function () {
      var first_item = $(".MainContent.is-visible")
        .find(".SlideFinancieras-item.slick-active")
        .find(".link-wrapper")
        .first();
  ...
  events.navigateSlideTo = function (nombre_entidad, nombre_slide) {
    ...
    var target_slide = $(".MainContent.is-visible").find(
      '.SlideFinancieras[data-nom="' + nombre_slide + '"].SlideFinancieras-list'
    );

    // Buscamos la posición del resultado seleccionado
    var item_slide_seleccionado = target_slide.find(
      '.link-wrapper .link[data-nom-entidad="' + nombre_entidad + '"]'
    );
    var pos_resultado = item_slide_seleccionado
      .parents(".SlideFinancieras-item")
      .index();
